...
class SC_Event_List {
	...
	private function html_event( &$event, &$a, $single_day_only=false ) {
		...
		$out .= '</h4></div>';
		// event presenter and affiliation
		$out .= '<div class="event-presenter"><h5>';
		$presenter = esc_attr($this->db->truncate($event->presenter, 20, $skip=true));
		$affiliate = esc_attr($this->db->truncate($event->affiliate, 20, $skip=true));
		$affiliateURL = esc_attr($this->db->truncate($event->affiliateURL, 20, $skip=true));
		$out .= $presenter . ', ' . '<a href="' . $affiliateURL . '">' . $affiliate . '</a>';
		$out .= '</h5></div>';
