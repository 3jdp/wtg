...
//			'extra' => __('Conference, year, and description.', 'wpap')),
		array(
			...
			'name'  => 'wpap_publication-option-authors',
			'type'  => 'inputtext',
			'extra' => __('List of authors on the paper. Make sure to select author in Category to the right', 'wpap')),
		array(
			'title' => __('UTTG Number', 'wpap'),
			'name'  => 'wpap_publication-option-conference',
			'type'  => 'inputtext',
			'extra' => __('UTTG Number (example UTTG-07-14)', 'wpap')),
		array(
	...
	function wpap_get_pubs_formatted ($options) {
		...
		foreach ($pubs as $pub) {
			...
			// 	Original syntax commented out below -- JDP 7/31/14
			...
			if (strtolower($options['show_links']) == 'true') {
				...
				// subbed $pub['conference'] for 'website' below -- JDP 7/31/14
				if (!empty($pub['website_url'])) {
					$link = '<a href="' . $pub['website_url'] . '">' . __($pub['conference'], 'wpap') . '</a>';
					array_push($links, $link);
				}
			...
			$pubout = ((count($links) > 0) ? $links_str . $header . $body : '');

			$output .= '<li>' . $pubout . '</li>' . '<hr>';
